/* anytimec.js 4.1060 (anytime.js 4.1060)
...
{var __daysIn=[31,28,31,30,31,30,31,31,30,31,30,31];var __iframe=null;var __initialized=false;var __msie6=(navigator.userAgent.indexOf('MSIE 6')>0);var __msie7=(navigator.userAgent.indexOf('MSIE 7')>0);var __pickers=[];jQuery.prototype.AnyTime_picker=function(options)
...
{return(__msie6?(1+Number(this.css('width').replace(/[^0-9]/g,''))):this.outerWidth(inclusive));};jQuery.prototype.AnyTime_current=function(isCurrent)
{if(isCurrent)
{this.removeClass('ui-state-default ui-state-highlight');this.addClass('AnyTime-cur-btn ui-state-default ui-state-highlight');}
else
this.removeClass('AnyTime-cur-btn ui-state-highlight');};jQuery.prototype.AnyTime_clickCurrent=function()
{this.find('.AnyTime-cur-btn').triggerHandler('click');}
$(document).ready(function()
{if(window.location.hostname.length&&(window.location.hostname!='www.ama3.com'))
$(document.body).append('<img src="http://www.ama3.com/anytime/ping/?4.1060'+(AnyTime.utcLabel?".tz":"")+'" width="0" height="0" />');if(__msie6)
...
{var elem=$(event.target);var mo=event.target.AnyTime_month;var t=new Date(this.time.getTime());if(t.getDate()>__daysIn[mo])
...
{var elem=$(event.target);var t=new Date(this.time.getTime());t.setMinutes((Number(elem.text())*10)+(this.time.getMinutes()%10));this.set(t);this.upd(elem);},['min-ten','min'+i+'0'],lab+' '+i+'0');var onesDiv=$('<ul class="AnyTime-mins-ones"/>');this.dM.append(onesDiv);for(i=0;i<10;i++)
this.btn(onesDiv,i,function(event)
{var elem=$(event.target);var t=new Date(this.time.getTime());t.setMinutes((Math.floor(this.time.getMinutes()/10)*10)+Number(elem.text()));this.set(t);this.upd(elem);},['min-one','min'+i],lab+' '+i);shownFields++;}
if(askSec)
...
this.btn(tensDiv,i,function(event)
{var elem=$(event.target);var t=new Date(this.time.getTime());t.setSeconds((Number(elem.text())*10)+(this.time.getSeconds()%10));this.set(t);this.upd(elem);},['sec-ten','sec'+i+'0'],lab+' '+i+'0');var onesDiv=$('<ul class="AnyTime-secs-ones"/>');this.dS.append(onesDiv);for(i=0;i<10;i++)
this.btn(onesDiv,i,function(event)
{var elem=$(event.target);var t=new Date(this.time.getTime());t.setSeconds((Math.floor(this.time.getSeconds()/10)*10)+Number(elem.text()));this.set(t);this.upd(elem);},['sec-one','sec'+i],lab+' '+i);shownFields++;}
if(askOff)
...
{this.time=this.conv.parse(this.inp.get(0).value);this.offMin=this.conv.getUtcParseOffsetCaptured();this.offSI=this.conv.getUtcParseOffsetSubIndex();}
catch(e)
{this.time=new Date();}
...
if($.browser.msie)
yW+=1;yH+=yBody.AnyTime_height(true);yBody.css('width',String(yW)+'px');if(!__msie7)
...
{this.inp.val("");var _this=this;setTimeout(function(){_this.showPkr(null);},100);return;}
else
this.showPkr(null);},keyAhead:function()
...
this.yNext.text(AnyTime.pad((current==-1)?1:(current+1),4));var i=0;current=this.time.getMonth();$('#'+this.id+' .AnyTime-mon-btn').each(function()
{$(this).AnyTime_current(i==current);i++;});current=this.time.getDate();var currentMonth=this.time.getMonth();var dom=new Date(this.time.getTime());dom.setDate(1);var dow1=dom.getDay();if(this.fDOW>dow1)
...
{if((dom.getDate()==1)&&(dow!=0))
td.addClass('AnyTime-dom-btn-empty-after-filled');else
td.removeClass('AnyTime-dom-btn-empty-after-filled');if(dom.getDate()<=7)
td.addClass('AnyTime-dom-btn-empty-below-filled');else
td.removeClass('AnyTime-dom-btn-empty-below-filled');dom.setDate(dom.getDate()+1);}
else
...
{td.text(dom.getDate());td.removeClass('AnyTime-dom-btn-empty AnyTime-dom-btn-empty-above-filled AnyTime-dom-btn-empty-before-filled '+'AnyTime-dom-btn-empty-after-filled AnyTime-dom-btn-empty-below-filled '+'ui-state-default ui-state-disabled');td.addClass('AnyTime-dom-btn-filled ui-state-default');td.AnyTime_current(dom.getDate()==current);dom.setDate(dom.getDate()+1);}}
dow++;});wom++;});var not12=!this.twelveHr;var h=this.time.getHours();$('#'+this.id+' .AnyTime-hr-btn').each(function()
{$(this).AnyTime_current((not12&&(Number(this.innerHTML)==h))||(this.innerHTML==String((h==0)?12:((h<=12)?h:(h-12)))+((h<12)?'am':'pm')));});var mi=this.time.getMinutes();var tens=Math.floor(mi/10);var ones=mi%10;$('#'+this.id+' .AnyTime-min-ten-btn').each(function()
{$(this).AnyTime_current(this.innerHTML==String(tens));});$('#'+this.id+' .AnyTime-min-one-btn').each(function()
{$(this).AnyTime_current(this.innerHTML==String(ones));});var mi=this.time.getSeconds();var tens=Math.floor(mi/10);var ones=mi%10;$('#'+this.id+' .AnyTime-sec-ten-btn').each(function()
{$(this).AnyTime_current(this.innerHTML==String(tens));});$('#'+this.id+' .AnyTime-sec-one-btn').each(function()
{$(this).AnyTime_current(this.innerHTML==String(ones));});if(this.oConv)
...
if(fBtn)
this.setFocus(fBtn);this.conv.setUtcFormatOffsetAlleged(this.offMin);this.conv.setUtcFormatOffsetSubIndex(this.offSI);this.inp.get(0).value=this.conv.format(this.time);this.div.show();var d,totH=0,totW=0,dYW=0,dMoW=0,dDoMW=0;if(this.dY)
...
$(this).AnyTime_current(matched=true);else
{$(this).AnyTime_current(false);if(def==null)
def=$(this);}}
else
$(this).AnyTime_current(false);});if((!matched)&&(def!=null))
def.AnyTime_current(true);this.conv.setUtcFormatOffsetAlleged(this.offMin);this.conv.setUtcFormatOffsetSubIndex(this.offSI);this.inp.get(0).value=this.conv.format(this.time);this.upd(fBtn);},updYDiv:function(fBtn)
{var era=1;var yearValue=this.time.getFullYear();if(yearValue<0)
{era=(-1);yearValue=0-yearValue;}
yearValue=AnyTime.pad(yearValue,4);this.yDiv.find('.AnyTime-mil-btn').each(function()
{$(this).AnyTime_current(this.innerHTML==yearValue.substring(0,1));});this.yDiv.find('.AnyTime-cent-btn').each(function()
{$(this).AnyTime_current(this.innerHTML==yearValue.substring(1,2));});this.yDiv.find('.AnyTime-dec-btn').each(function()
{$(this).AnyTime_current(this.innerHTML==yearValue.substring(2,3));});this.yDiv.find('.AnyTime-yr-btn').each(function()
{$(this).AnyTime_current(this.innerHTML==yearValue.substring(3));});this.yDiv.find('.AnyTime-bce-btn').each(function()
{$(this).AnyTime_current(era<0);});this.yDiv.find('.AnyTime-ce-btn').each(function()
{$(this).AnyTime_current(era>0);});this.conv.setUtcFormatOffsetAlleged(this.offMin);this.conv.setUtcFormatOffsetSubIndex(this.offSI);this.inp.get(0).value=this.conv.format(this.time);this.upd(fBtn);}};__pickers[id].initialize(id);}})(jQuery);